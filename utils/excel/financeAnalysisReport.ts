...
import type { FinanceAnalysisData, FinanceAnalysisReceiptBreakdownItem } from '~/types/financeAnalysis'
...
function buildOverviewRows(options: FinanceAnalysisReportOptions) {
  ...
  const receiptStateRows = [
    { label: receiptStatusLabels.paid, count: summary.receipt_paid_count, total: summary.receipt_paid_total },
    { label: receiptStatusLabels.open, count: summary.receipt_open_count, total: summary.receipt_open_total },
    { label: receiptStatusLabels.draft, count: summary.receipt_draft_count, total: summary.receipt_draft_total },
    { label: receiptStatusLabels.cancelled, count: summary.receipt_cancelled_count, total: summary.receipt_cancelled_total },
  ]
  ...
  const rows = [
    ...
    createSpreadsheetRow([
      ...
      {
        value: selectedStatuses.map(status => receiptStatusLabels[status]).join(', ') || t('financeAnalysis.noneSelected'),
        styleId: 'Body',
  ...
  if (hasComparison && comparisonAnalysis) {
    ...
    const entryDifference = (summary.receipt_count + summary.cash_count_count) - (comparisonAnalysis.summary.receipt_count + comparisonAnalysis.summary.cash_count_count)
    ...
    rows.push(
      ...
      createSpreadsheetRow([
        ...
        countCell(summary.receipt_count + summary.cash_count_count),
        countCell(comparisonAnalysis.summary.receipt_count + comparisonAnalysis.summary.cash_count_count),
    ...
    rows.push(
      ...
      createSpreadsheetRow([{ value: t('financeAnalysis.cards.entriesReviewed'), styleId: 'TextCell' }, countCell(summary.receipt_count + summary.cash_count_count, 'CountCell', 1)]),
    )
  }
  ...
  rows.push(
    ...
    createSpreadsheetRow([
      { value: t('financeAnalysis.receiptStateFilters'), styleId: 'Header' },
  ...
  rows.push(
    ...
    createSpreadsheetRow([{ value: t('financeAnalysis.cashCards.totalBefore'), styleId: 'TextCell' }, currencyCell(summary.cash_count_total_before, 'CurrencyCell', valueMergeAcross)]),
    createSpreadsheetRow([{ value: t('financeAnalysis.cashCards.totalAfter'), styleId: 'TextCell' }, currencyCell(summary.cash_count_total_after, 'CurrencyCell', valueMergeAcross)]),
    createSpreadsheetRow([{ value: t('financeAnalysis.cashCards.registers'), styleId: 'TextCell' }, countCell(summary.cash_count_register_total, 'CountCell', valueMergeAcross)]),
  )
...
function scaleColumnWidthsToTotal(widths: number[], targetTotal: number) {
  ...
  scaledWidths[scaledWidths.length - 1] += difference
