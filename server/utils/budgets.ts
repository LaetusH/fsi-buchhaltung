...
export function getBudgetPeriod(startDate: string, endDate: string): { year: number, semester: BudgetSemester } {
  const [startYear, startMonth] = startDate.split('-').map(Number) as [number, number]
  const [, endMonth] = endDate.split('-').map(Number) as [number, number]
...
export async function loadBudgetDetail(budgetId: number): Promise<BudgetDetail | null> {
