<template>
  <nav class="space-y-4 text-sm">
    ...
    <div v-for="space in spaces" :key="space.id" class="space-y-1">
      <button
        type="button"
        class="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left font-semibold text-slate-800 hover:bg-slate-100 cursor-pointer"
      ...
      >
        <Icon
          ...
          aria-hidden="true"
        />
        <Icon :name="space.icon" class="h-4 w-4 shrink-0 text-slate-500" aria-hidden="true" />
        <span class="min-w-0 truncate">{{ space.title }}</span>
      </button>

      <ul v-if="expandedSpaces.has(space.id)" class="space-y-0.5 pl-3">
